...
public class Graphe {
    ...
    private Vector<Noeud> noeuds = new Vector<Noeud>() ;
    private Vector<Descripteur> descripteurs = new Vector<Descripteur>();
	...
	try {
	    ...
	    for (int num_node = 0 ; num_node < nb_nodes ; num_node++) {
			...
			float lon = ((float)dis.readInt ()) / 1E6f ;
			float lat = ((float)dis.readInt ()) / 1E6f ;
			byte nb = (byte)dis.readUnsignedByte();
	    	noeuds.add(new Noeud(num_node,lat,lon,nb));
	    }
	    ...
	    for (int num_descr = 0 ; num_descr < nb_descripteurs ; num_descr++) {
			...
			if (0 == num_descr % (1 + nb_descripteurs / 400))
			    System.out.println("Descripteur " + num_descr + " = " + descripteurs.get(num_descr)) ;
	    }
	    ...
	    for (int num_node = 0 ; num_node < nb_nodes ; num_node++) {
			...
			for (int num_succ = 0 ; num_succ < noeuds.get(num_node).getNbSuccesseur() ; num_succ++) {
			    // zone du successeur
			    ...
			    Couleur.set(dessin, descripteurs.get(descr_num).getType()) ;
	
			    float current_long = noeuds.get(num_node).getLongitude();
			    float current_lat  = noeuds.get(num_node).getLatitude();
			    Descripteur descripteurCourrant = descripteurs.get(descr_num);
			    noeuds.get(num_node).addSuccesseur(noeuds.get(dest_node),descripteurCourrant,succ_zone,(short)longueur,(short)nb_segm);
			    if(!descripteurCourrant.isSensUnique()) {
				    noeuds.get(dest_node).addSuccesseur(noeuds.get(num_node),descripteurCourrant,succ_zone,(short)longueur,(short)nb_segm);
			    }
			    ...
			    for (int i = 0 ; i < nb_segm ; i++) {
					...
					noeuds.get(num_node).getLastSuccesseur().addSegment(new Segment(delta_lon,delta_lat));
					if(!descripteurCourrant.isSensUnique()) {
						noeuds.get(dest_node).getLastSuccesseur().addSegment(new Segment(delta_lon,delta_lat));
				    }
	...
	try {
	    ...
	    chemin = new Chemin(magic,version,path_carte,nb_noeuds,noeuds.get(first_node), noeuds.get(last_node),first_zone,last_zone);
	    ...
	    for (int i = 0 ; i < nb_noeuds ; i++) {
			...
			chemin.addNoeud(noeuds.get(current_node));
	    ...
	    System.out.println("Temps pour parcourrir le chemin :"+chemin.getCoutCheminTemps()+"\n");

	 // Dessin
	    dessin.setColor(new Color(255,13,255));
	    float it1;
	    float it2;
	    for(int j=0; j<chemin.getNoeudsChemin().size(); j++) {
	    	it1 = (float) (chemin.getNoeudsChemin().get(j).getLongitude());
	    	it2 = (float) (chemin.getNoeudsChemin().get(j).getLatitude());
	    	dessin.drawPoint(it1, it2, 5);   	
	    }
